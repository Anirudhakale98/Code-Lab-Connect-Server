...
const registerUser = asyncHandler(async (req, res) => {
    const { firstName, lastName, email, password, role, prn, rollNo } = req.body;

    // Validate required fields
    if (![firstName, lastName, email, password, role].every(Boolean)) {
        throw new ApiError(400, "All required fields must be provided");
    }

    // Additional validation for Student role
    if (role === "student" && (!prn || !rollNo)) {
        throw new ApiError(400, "PRN and Roll Number are required for student");
    }
